...
\begin{itemize}
    \item $R$ and $T$ are \hlr{unknown}
    \item Access to a \hlb{generative model} $s' \sim \probability{s'|s,a}$ and $r\sim\probability{r|s, a}$
    \item \hlr{Fixed-budget} setting: the generative model can be queried $n$ times
\end{itemize}
}
