...
%
\author{Edouard Leurent\inst{1,2}\and
Odalric-Ambrym Maillard\inst{1}}
%
\authorrunning{E. Leurent, O-A. Maillard}
...
\begin{itemize}
    \item First, we show that in our experiments \OLOP is overly pessimistic, especially in the low-budget regime, and we provide an intuitive explanation by showcasing an unintended effect that alters the behaviour of \OLOP.
...
\begin{figure}[tp]
    \centering
    \includegraphics[width=0.6\textwidth]{img/ukl.eps}
...
\begin{equation*}
    \label{eq:kappa}
    \kappa_2 = \limsup_{h\rightarrow\infty}{\left|\left\{a\in a^H:V(a) \geq V - 2\frac{\gamma^{h+1}}{1-\gamma}\right\}\right|^{1/h}}
\end{equation*}
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.6\textwidth]{img/tree}
...
\begin{itemize}
    ...
    \end{equation*}
    For the last term to be of the same order of the others, we need to have $\Delta_M = O(M^{-\frac{\log \kappa'}{2\log 1/\gamma}})$. Since $\kappa'\gamma^2 \leq 1$, this can be achieved by taking \diff{$\Delta_M = O(M^{-1})$}.
...
\begin{itemize}
    \item \texttt{Random}: returns an action at random. We use it as a baseline for minimal performance.
...
\begin{minipage}{\textwidth}
    \subfloat[Highway]{
      \includesvg[width=\textwidth]{img/hw_return.svg}
      \label{sub:highway}
    }
    \newline
    \subfloat[Gridworld]{
      \includesvg[width=0.5\textwidth]{img/gw_return.svg}
      \label{sub:gridworld}
    }
    \subfloat[Stochastic gridworld]{
      \includesvg[width=0.5\textwidth]{img/gw_stoch_return.svg}
...
\begin{proof}
See proof of Lemma 4 in \citep{Bubeck2010}.
\qed
\end{proof}
...
\begin{proof}
The proof is identical to that of Lemma 9 in \citep{Bubeck2010}, where they show:

\begin{equation*}
    |\mathcal{P}^\emptyset_{h,h'}(M)| \leq \mathbbm{1}_{h'>0}\sum_{t=0}^{h'}(\gamma^{2(t-h')}|\mathcal{I}_t| + \sum_{m=1}^{M}\sum_{a\in \mathcal{J}_h} \mathbbm{1}\{\eqref{eq:cond-ukl}\text{ or }\eqref{eq:cond-lkl}\}
\end{equation*}
Taking the expectation and applying Lemma \ref{lemma:boundary-crossing-prob} yield the claimed bound.
\qed
\end{proof}
...
\begin{proof}
The proof is identical to that of Lemma 10 in \citep{Bubeck2010}, where they show:
\begin{equation*}
    \sum_{a\in \mathcal{J}_h} T_a(M) = \tilde{O}\left((\kappa')^h + \gamma^{-2h}\sum_{h'=1}^{h-1} \gamma^{2h'}|\mathcal{P}^\emptyset_{h,h'}(M)| + M|\mathcal{P}^\emptyset_{h,0}(M)|\right)
\end{equation*}
